...
setwd( R'(C:\Users\James.Thorson\Desktop\Work files\Collaborations\2025 -- Wassersmann AT-BT)' )
...
# Add catchability term ... actually just doing it manually in R function below
#dat$unit = ifelse( dat$Gear == "AVO2", "backscatter", "biomass" )
#dat$unit = factor( dat$unit, levels = c("biomass", "backscatter") )
...
# Get from shapefile
shape_dir = R'(C:\Users\James.Thorson\Desktop\Work files\AFSC\2025-01 -- ridge.correct for acoustics and bottom trawl)'
#shape_dir = getwd()
...
extrap = cbind( 'Lon'=extrap[,1], 'Lat'=extrap[,2],
...
jnll_spde = function( parlist, what="jnll" ){
  ...
  for( c_index in 1:3 ){
    if( t_index==1 ){
      nll_epsilon = nll_epsilon - dgmrf( epsilon_sct[,c_index,t_index], Q = Q_epsilon, log=TRUE )
    }else{
      nll_epsilon = nll_epsilon - dgmrf( epsilon_sct[,c_index,t_index], mu = rho * epsilon_sct[,c_index,t_index-1], Q = Q_epsilon, log=TRUE )
    }
  }}
  for( c_index in 1:3 ){
    nll_omega = nll_omega - dgmrf( omega_sc[,c_index], Q = Q_omega, log=TRUE )
  }
  for( t_index in 1:max(t_i) ){
  for( c_index in 1:3 ){
    if( t_index==1 ){
...
png( file="Fig_5_comparison.png", width=5, height=6, res=200, units="in" )
...
for(c_index in 1:3 ){
  png( file=paste0("Densities_",c("low","med","hi")[c_index],".png"), width=7.5, height=6, units="in", res=200 )
    par( mfrow=c(3,4) )
...
for(c_index in 1:3 ){
  png( file=paste0("eps_",c("low","med","hi")[c_index],".png"), width=7.5, height=6, units="in", res=200 )
    par( mfrow=c(3,4) )
...
#
for(c_index in 1:4 ){
  png( file=paste0("Densities_",c("BT","AT","BTprop","ATprop")[c_index],".png"), width=7.5, height=6, units="in", res=200 )
    par( mfrow=c(3,4) )
    if( c_index %in% 1:2 ){
